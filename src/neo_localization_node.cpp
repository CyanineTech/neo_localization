...
{
public:
  NeoLocalizationNode() : m_node_handle("~")  // Change to private node handle
  {
	...
	bool has_initial_x = m_node_handle.getParam("initial_pose_x", initial_pose_x);
	bool has_initial_y = m_node_handle.getParam("initial_pose_y", initial_pose_y);
	bool has_initial_a = m_node_handle.getParam("initial_pose_a", initial_pose_a);

	// Set initial pose if all parameters are available
	if (has_initial_x && has_initial_y && has_initial_a)
	{
	  ROS_INFO("Setting initial pose from parameters: x=%.3f, y=%.3f, yaw=%.3f", initial_pose_x, initial_pose_y,
			   initial_pose_a);
	  set_initial_pose(initial_pose_x, initial_pose_y, initial_pose_a);
	}
	else
	{
	  ROS_INFO("Initial pose parameters not found or incomplete. Waiting for manual initialization.");
	}
...
protected:
   ...
   */
  std::vector<scan_point_t> convert_scan(const sensor_msgs::LaserScan::ConstPtr& scan,
										 const Matrix<double, 4, 4>& odom_to_base)
  {
	...
	{
	  ROS_WARN_STREAM(
		  "NeoLocalizationNode: lookupTransform(scan->header.frame_id, m_base_frame) failed: " << ex.what());
	  return points;
	}
  ...
  void loc_update(const ros::TimerEvent& event)
  {
	...
	const Matrix<double, 3, 1> grid_pose =
		(m_grid_to_map.inverse() * T * L * Matrix<double, 4, 1>{ 0, 0, 0, 1 }).project();
	...
	const Matrix<double, 3, 1> grad_std_uvw{ sqrt(grad_eigen_values[0]), sqrt(grad_eigen_values[1]),
	...
	const Matrix<double, 3, 1> new_map_pose =
		(translate25(m_offset_x, m_offset_y) * rotate25_z(m_offset_yaw) * L * Matrix<double, 4, 1>{ 0, 0, 0, 1 })
			.project();
	...
	{
	  ROS_INFO_STREAM("NeoLocalizationNode: score="
					  << float(best_score) << ", grad_uvw=[" << float(grad_std_uvw[0]) << ", " << float(grad_std_uvw[1])
					  << ", " << float(grad_std_uvw[2]) << "], std_xy=" << float(m_sample_std_xy) << " m, std_yaw="
					  << float(m_sample_std_yaw) << " rad, mode=" << mode << "D, " << m_scan_buffer.size() << " scans");
	}
  ...
  {
	{
	  ...
	  ROS_INFO_STREAM("NeoLocalizationNode: Got new map pose estimate: x="
					  << map_pose.getOrigin()[0] << " m, y=" << map_pose.getOrigin()[1]
  ...
  void set_initial_pose(double x, double y, double yaw)
  {
	...
	// Set initial offset values (these represent the transform from odom to map)
	m_offset_x = x;
	m_offset_y = y;
	m_offset_yaw = yaw;
  ...
  void map_callback(const nav_msgs::OccupancyGrid::ConstPtr& ros_map)
  {
	...
	ROS_INFO_STREAM("NeoLocalizationNode: Got new map with dimensions " << ros_map->info.width << " x "
																		<< ros_map->info.height << " and cell size "
  ...
  void update_map()
  {
	...
	ROS_DEBUG_STREAM("NeoLocalizationNode: Got new grid at offset (" << tile_x << ", " << tile_y
																	 << ") [iworld], "
																		"center = ("
